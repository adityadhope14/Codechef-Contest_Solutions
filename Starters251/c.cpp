...
void solve() {
    ...
    string s;
    cin >> s;
    vector<pair<char, int>> blcks;
    int curr_cnt;
    for(int i = 1; i < n; i++) {
        if (s[i] == s[i-1]) {
            curr_cnt++;
        } else {
            blcks.push_back({s[i-1], curr_cnt});
            curr_cnt = 1;
        }
    }
    blcks.push_back({s[n-1], curr_cnt});
    if (blcks.size() == 1) {
        cout << s << endl;
        return;
    }
    if (blcks.size() == 2 && blcks[0].first == '1') {
        cout << s << endl;
        return;
    }
    int max_z = -1;
    int min_o = 1e9; 
    for (int i = 0; i < blcks.size() - 1; i++) {
        if (blcks[i].first == '0' && blcks[i+1].first == '1') {
            int curr_z = blcks[i].second;
            int curr_o = blcks[i+1].second;
            if (curr_z > max_z) {
                max_z = curr_z;
                min_o = curr_o;
            } 
    ...
    for (int i = 0; i < max_z; i++) cout << '0';
    for (int i = 0; i < min_o; i++) cout << '1';
    cout << endl;
}
// void solve() {
//     int n ;
